...
void ssd1306::init_SSD1306()
{
    ...
    send_command_SSD1306(SSD1306_MUX_128x32); // Change to SSD1306_MUX_128x32 for 128x32 displays
    ...
    send_command_SSD1306(SSD1306_COM_PINS_128x32); // Change to SSD1306_COM_PINS_128x32 for 128x32 displays
...
void ssd1306::draw_pixel_SSD1306(uint8_t x, uint8_t y)
{
    ...
    uint8_t pixelInPage = (1 << (y % (SSD1306_page_height))); //limit the pixel from 0 to 7 with a 1 in the position 0000_0001, 0000_0010, etc
...
void ssd1306::clear_buffer()
{
    ...
    memset(buffer, 0, SSD1306_BUFFER_SIZE_128x32);
    return;
}