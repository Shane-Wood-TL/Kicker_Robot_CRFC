...
void ssd1306::init_SSD1306() {
  ...
  send_command_SSD1306(SSD1306_MUX_128x64); // Change to SSD1306_MUX_128x32 for 128x32 displays
  ...
  send_command_SSD1306(SSD1306_COM_PINS_128x64); // Change to SSD1306_COM_PINS_128x32 for 128x32 displays
...
void ssd1306::write_buffer_SSD1306() {
  for (uint8_t i = 0; i < SSD1306_pages; i++) {       // Send each page
...
void ssd1306::write_letter_SSD1306(uint8_t letter, uint8_t x_pos, uint8_t y_pos) {
  ...
  for (int i = 0; i < font_height; i++) {                    // 16 tall
    for (int j = 0; j < font_width; j++) {                   // 12 wide
...
void ssd1306::clear_buffer() {
  ...
  memset(buffer, 0, SSD1306_BUFFER_SIZE_128x64);
  return;
}