...
void menu_handler::update(){
    if(xSemaphoreTake(update_main_display_mutex, pdMS_TO_TICKS(25))){
        if(update_main_display==true && current_state ==0){
            update_main_display = false;
            xSemaphoreGive(update_main_display_mutex);
            if(xSemaphoreTake(main_menu_values_mutex, pdMS_TO_TICKS(25))){
                draw_status_display();
                xSemaphoreGive(main_menu_values_mutex);
            }
    ...
    if((xQueueReceive(other_controller_data_queue,&current_data,pdMS_TO_TICKS(25)) == pdTRUE)){
        xQueueReset(other_controller_data_queue); //prevents multiple inputs
        bool current_options_state = current_data.options_triggers & (1 <<0);

        bool current_r2_state = current_data.options_triggers & (1 <<1);
        bool current_l2_state = current_data.options_triggers & (1 <<2);

        bool current_square_state = (current_data.buttons & (1 << 5));
        bool current_triangle_state = (current_data.buttons & (1 << 4));
        bool current_circle_state = (current_data.buttons & (1 << 6));

        bool current_dpad_up_state = current_data.buttons & (1 << 0);
        bool current_dpad_down_state = current_data.buttons & (1 << 2);
        bool current_dpad_left_state = current_data.buttons & (1 << 3) ;
        bool current_dpad_right_state = current_data.buttons & (1 << 1);
