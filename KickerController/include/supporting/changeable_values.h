...
template<typename T>
changeable_values<T>::changeable_values(T *value, T min_value, T max_value, T change_by) { //constructor, sets parameters to member variables
  this->value = value;
...
template<typename T>
void changeable_values<T>::increase_value() { //increase the value by change_by if it is within its bounds
  if (*value < max_value) {
...
template<typename T>
void changeable_values<T>::decrease_value() { //decrease the value by change_by if it is within its bounds
  if (*value > min_value) {
    *value = *value - change_by;
  }
  if (*value < min_value) {
...
template<typename T>
T changeable_values<T>::get_value() { //return the value of the variable value
  return *value;
}
