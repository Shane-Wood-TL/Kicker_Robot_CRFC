 ...
 * @details This class will is specific to the current hardware setup of the
 * controller, as button presses are used to switch between specific menus (such
 * as the right trigger releasing the servo) and modifications will need to be
 * done to utlize this class in other setups.
 */
class menu_handler {
private:
  enum rolling_menu_states {
    STATUS_SCREEN_STATE,
    MOTOR_SPEEDS_STATE,
    RAMPED_VELOCITY_STATE,
    NETWORK_CHAN
  };
  ...
  menu<changeable_values<uint8_t>>
    *network_channel_selector; ///< The networ channel selection menu

  uint8_t motors_enabled; ///< The current state of the motors (only enabled
  ///< / disabled)
  /**
  ...
  /**
    * @brief This function cycles throught the status, motor speed, and ramped
    * velocity menu menus
    */
  void options_pressed();

  /**
    * @brief This function will switch which variable can be modified in the
    * menus (if modifiable)
    * @details attempts to move the cursor one value down, will roll over if at
  ...
  /**
    * @brief This function will switch which variable can be modified in the
    * menus (if modifiable)
    * @details attempts to move the cursor one value up, will roll over if at
    * the top of the menu
    */
  void dpad_down_pressed();

  /**
    * @brief This function will decrease the value of the selected variable if
    * modifiable)
  ...
  /**
    * @brief This function will increase the value of the selected variable if
    * modifiable)
    * @details this value that it will be changed by is set in the menu class
    */
...
public:
  /**
    ...
    */
  menu_handler(ssd1306 *display, menu<std::string> *status_screen, menu<changeable_values<uint8_t>> *motor_speed_menu, menu<changeable_values<float>> *ramped_velocity_menu, menu<uint8_t> *servo_latched_menu, menu<uint8_t> *servo_released_menu, menu<uint8_t> *motors_calibrating_menu, menu<uint8_t> *motors_enable_menu, menu<uint8_t> *motors_disable_menu, menu<uint8_t> *errors_clear_menu, menu<changeable_values<uint8_t>> *network_channel_selector);
  /**
